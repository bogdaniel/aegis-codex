...
- Added `[CROSS-REFERENCES]` blocks to `rules/00-persona.mdc`, `rules/10-global.mdc`, `rules/11-meta-map.mdc`, `rules/20-agents.mdc`, `rules/21-orchestration.mdc`, `rules/22-ai-assistants.mdc`, `rules/23-change-control.mdc`, `rules/30-security.mdc`, `rules/30-threat-modeling.mdc`, and `rules/31-testing.mdc` pointing to canonical architecture/testing and risk override rules; no semantic changes.
- Added `[CROSS-REFERENCES]` blocks to `rules/32-observability.mdc`, `rules/34-ci.mdc`, `rules/35-api.mdc`, `rules/36-architecture.mdc`, `rules/38-anti-corruption-events.mdc`, `rules/38-compliance.mdc`, `rules/39-observability-security.mdc`, `rules/3B-data-persistence.mdc`, `rules/3D-operations.mdc`, and `rules/3E-config-environments.mdc` pointing to canonical architecture/testing and risk override rules; no semantic changes.
- Added `[CROSS-REFERENCES]` blocks to `rules/3F-feature-flags-rollouts.mdc`, `rules/3G-risk-overrides.mdc`, `rules/40-atdd.mdc`, `rules/44-ddd.mdc`, `rules/45-bugfix-protocol.mdc`, `rules/46-regression-discipline.mdc`, `rules/47-diff-discipline.mdc`, `rules/48-doc-sync.mdc`, `rules/50-lang-csharp.mdc`, and `rules/50-lang-css.mdc` pointing to canonical architecture/testing and risk override rules; no semantic changes.
- Added `[CROSS-REFERENCES]` blocks to remaining language files `rules/50-lang-{gdscript,go,html,java,javascript,php-laravel-guidelines,php,python,rust,typescript}.mdc` pointing to canonical architecture/testing and risk override rules; no semantic changes.
