/**
 * Aegis Agent Compiler
 * Transforms agent YAML to compiled XML (.md) format
 * Uses the existing Aegis builder infrastructure for proper formatting
 */
...
// Use existing Aegis builder if available
let YamlXmlBuilder;
try {
...
function buildFrontmatter(metadata, agentName) {
  ...
  const description = metadata.title || 'Aegis Agent';
...
function buildSimpleActivation(criticalActions = [], menuItems = [], deploymentType = 'ide') {
  ...
  activation += `  <step n="${stepNum++}">Load and read {project-root}/{aegis_folder}/core/config.yaml to get {user_name}, {communication_language}, {output_folder}</step>\n`;
