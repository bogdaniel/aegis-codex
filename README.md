...
## Layout
- `AGENTS.md` — Human‑readable aggregation of all rules, generated from `.cursor/rules/*.mdc` (do not edit by hand; see [Rule Builder](#rule-builder) section).
- `.cursor/rules/*.mdc` — Machine‑readable rules for Cursor; always‑on standards. Generated by the rule builder based on your project configuration.
