/**
 * Example Wiring - Dependency Injection Container
 * Shows how to wire up all layers and contexts.
...
// Example usage (would be called from HTTP server routes)
export async function exampleUsage() {
  ...
  const registerRes = await identityHandlers.handleRegister(registerReq);
  ...
  const authRes = await identityHandlers.handleAuthenticate(authReq);
  ...
  const placeOrderRes = await ordersHandlers.handlePlaceOrder(placeOrderReq);
  console.log('Place order response:', placeOrderRes);

  // Get order summary
  const orderId = JSON.parse(placeOrderRes.body as string).orderId;
  const summaryReq = {
    body: { userId: 'user_123' },
    headers: {},
    params: { orderId },
  };
  const summaryRes = await ordersHandlers.handleGetOrderSummary(summaryReq);
  console.log('Order summary:', summaryRes);
}
