...
```
Read: {project-root}/{aegis_folder}/builder/reference/workflows/meal-prep-nutrition/workflow.md
```
...
- **Linear**: Like Meal Prep Plan (Init → Profile → Assessment → Strategy → Shopping → Prep)
  - See: `{project-root}/{aegis_folder}/builder/reference/workflows/meal-prep-nutrition/`
- **Looping**: User Story Generator (Generate → Review → Refine → Generate more... until done)
- **Branching**: Architecture Deinnovationion (Analyze → Choose pattern → Implement based on choice)
...
- Are there specific deinnovationion points where user must choose?
...
- What is the primary output (document, action, deinnovationion)?
