...
/**
 * Aegis Builder Module Installer
 * Sets up custom agent and workflow locations for the Aegis Builder module
 *
...
async function install(options) {
  ...
  try {
    logger.log(chalk.blue('🔧 Setting up Aegis Builder Module...'));
    ...
    if (config['custom_stand_alone_location']) {
      ...
      const customContent = `code: my-custom-aegis
name: "${userName}-Custom-Aegis: Sample Stand Alone Custom Agents and Workflows"
...
`;
    ...
    if (installedIDEs && installedIDEs.length > 0) {
      logger.log(chalk.cyan(`  Configuring Aegis Builder for IDEs: ${installedIDEs.join(', ')}`));
    }

    logger.log(chalk.green('✓ Aegis Builder Module setup complete'));
    return true;
  } catch (error) {
    logger.error(chalk.red(`Error setting up Aegis Builder module: ${error.message}`));
    return false;
  }
}
