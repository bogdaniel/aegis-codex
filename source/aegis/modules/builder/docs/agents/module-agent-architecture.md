...
- Module-specific functionality (Aegis Method, Aegis Builder, Aegis Innovation, custom modules)
...
```
.aegis/{module-code}/agents/{agent-name}.md
```
...
agent:
  metadata:
    id: '{*aegis_folder*}/{module-code}/agents/{agent-name}.md'
    name: 'Persona Name'
  ...
  menu:
    - trigger: workflow-action
      workflow: '{project-root}/{*aegis_folder*}/{module-code}/workflows/{workflow-name}/workflow.yaml'
      description: 'Execute module workflow'

    - trigger: another-workflow
      workflow: '{project-root}/{*aegis_folder*}/core/workflows/{workflow-name}/workflow.yaml'
      description: 'Execute core workflow'

    - trigger: task-action
      exec: '{project-root}/{*aegis_folder*}/{module-code}/tasks/{task-name}.xml'
      description: 'Execute module task'

    - trigger: cross-module
      workflow: '{project-root}/{*aegis_folder*}/other-module/workflows/{workflow-name}/workflow.yaml'
      description: 'Execute workflow from another module'

    - trigger: with-template
      exec: '{project-root}/{*aegis_folder*}/core/tasks/create-doc.xml'
      tmpl: '{project-root}/{*aegis_folder*}/{module-code}/templates/{template-name}.md'
      description: 'Create document from template'

    - trigger: with-data
      exec: '{project-root}/{*aegis_folder*}/{module-code}/tasks/{task-name}.xml'
      data: '{project-root}/{*aegis_folder*}/_cfg/agent-manifest.json'
...
- **id**: Path with `{*aegis_folder*}` variable (resolved at install time)
- **name**: Agent persona name
- **title**: Professional role
- **icon**: Single emoji
- **module**: Module code (method, builder, innovation, custom)
...
persona:
  ...
  communication_style: Systematic and probing. Connects dots others miss. Structures findings hierarchically. Uses preinnovatione unambiguous language. Ensures all stakeholder voices heard.

  principles: Every business challenge has root causes waiting to be discovered. Ground findings in verifiable evidence. Articulate requirements with absolute preinnovationion.
```
...
menu:
  - trigger: create-prd
    workflow: '{project-root}/{*aegis_folder*}/method/workflows/prd/workflow.yaml'
    description: 'Create Product Requirements Document'
```

Invokes Aegis workflow engine to execute multi-step processes.
...
menu:
  - trigger: validate
    exec: '{project-root}/{*aegis_folder*}/core/tasks/validate-workflow.xml'
...
menu:
  - trigger: create-brief
    exec: '{project-root}/{*aegis_folder*}/core/tasks/create-doc.xml'
    tmpl: '{project-root}/{*aegis_folder*}/method/templates/brief.md'
...
menu:
  - trigger: team-standup
    exec: '{project-root}/{*aegis_folder*}/method/tasks/standup.xml'
    data: '{project-root}/{*aegis_folder*}/_cfg/agent-manifest.json'
...
menu:
  - trigger: advanced-elicitation
    exec: '{project-root}/{*aegis_folder*}/core/tasks/advanced-elicitation.xml'
    description: 'Advanced elicitation techniques'
    web-only: true # Only shows in web bundle

  - trigger: git-operations
    exec: '{project-root}/{*aegis_folder*}/method/tasks/git-flow.xml'
...
- `{*aegis_folder*}` - Aegis installation folder (usually `.aegis`)
...
# BAD
workflow: "/Users/john/project/.aegis/method/workflows/prd/workflow.yaml"

# BAD
workflow: "../../../method/workflows/prd/workflow.yaml"
```
...
```xml
<step n="2">Load and read {project-root}/{*aegis_folder*}/{module}/config.yaml...</step>
```

## Reference Examples

See: `src/modules/method/agents/`
...
- Architecture deinnovationion workflows
...
menu:
  - trigger: init
    workflow: '{project-root}/{*aegis_folder*}/method/workflows/workflow-init/workflow.yaml'
    description: 'Initialize workflow path (START HERE)'

  - trigger: status
    workflow: '{project-root}/{*aegis_folder*}/method/workflows/workflow-status/workflow.yaml'
    description: 'Check current workflow status'

  - trigger: next-step
    workflow: '{project-root}/{*aegis_folder*}/method/workflows/next-step/workflow.yaml'
...
menu:
  # Phase 1: Analysis
  - trigger: brainstorm
    workflow: '{project-root}/{*aegis_folder*}/method/workflows/1-analysis/brainstorm/workflow.yaml'
    description: 'Guided brainstorming session'

  - trigger: research
    workflow: '{project-root}/{*aegis_folder*}/method/workflows/1-analysis/research/workflow.yaml'
    description: 'Market and technical research'

  # Phase 2: Planning
  - trigger: prd
    workflow: '{project-root}/{*aegis_folder*}/method/workflows/2-planning/prd/workflow.yaml'
    description: 'Create PRD'

  - trigger: architecture
    workflow: '{project-root}/{*aegis_folder*}/method/workflows/2-planning/architecture/workflow.yaml'
...
menu:
  - trigger: party-mode
    workflow: '{project-root}/{*aegis_folder*}/core/workflows/party-mode/workflow.yaml'
    description: 'Bring all agents together'

  - trigger: brainstorm
    workflow: '{project-root}/{*aegis_folder*}/innovation/workflows/brainstorming/workflow.yaml'
    description: 'Use Aegis Innovation brainstorming techniques'
```

## Best Practices

1. **Use {_aegis_folder_} paths** - Portable across installations
...
menu:
  - trigger: start
    workflow: '{project-root}/{*aegis_folder*}/{module}/workflows/init/workflow.yaml'
...
menu:
  - trigger: status
    workflow: '{project-root}/{*aegis_folder*}/{module}/workflows/status/workflow.yaml'
...
| ------------- | ---------------------------------- | -------------------------------- |
| Location      | `{*aegis_folder*}/{module}/agents/` | `{*aegis_folder*}/custom/agents/` |
...
- [ ] id uses `{*aegis_folder*}/{module}/agents/{name}.md`
- [ ] All workflow paths use `{project-root}/{*aegis_folder*}/` prefix
- [ ] No hardcoded paths
