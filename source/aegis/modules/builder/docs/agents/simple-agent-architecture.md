...
agent:
  metadata:
    id: .aegis/agents/{agent-name}/{agent-name}.md
    name: 'Persona Name'
    title: 'Agent Title'
    icon: 'emoji'
    type: simple
  ...
  persona:
    ...
    principles:
      ...
      - Values that shape deinnovationions
...
- **id**: Final compiled path (`.aegis/agents/{name}/{name}.md` for standalone)
...
Questions asked during `aegis agent-install`:
...
See: `src/modules/builder/reference/agents/simple-examples/commit-poet.agent.yaml`
...
cp /path/to/commit-poet.agent.yaml .aegis/custom/agents/
