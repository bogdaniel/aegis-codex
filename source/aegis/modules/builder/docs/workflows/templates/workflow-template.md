# Aegis Workflow Template

This template provides the standard structure for all Aegis workflow files. Copy and modify this template for each new workflow you create.
...
Load and read full config from {project-root}/{_aegis_folder_}/[MODULE FOLDER]/config.yaml and resolve:
...
- `[MODULE FOLDER]` → Default is `core` unless this is for another module (such as method, innovation, or another as directed by user)
...
- `[CONFIG_PATH]` → Path to config file (usually `method/config.yaml` or `builder/config.yaml`)
...
`{project-root}/{aegis_folder}/builder/reference/workflows/meal-prep-nutrition/workflow.md`
