...
- **Data-Driven Analysis**: Uses JSON-based project type detection and documentation requirements
...
```bash
/aegis:method:workflows:document-project
```
...
The workflow uses a single comprehensive JSON file:

**documentation-requirements.json** - Complete project analysis guide

- Location: `/{aegis_folder}/method/workflows/document-project/documentation-requirements.json`
...
- Modify `documentation-requirements.json` to adjust scanning patterns for project types
- Add new project types to `project-types.json`
- Add new architecture templates to `registry.json`
...
├── documentation-requirements.json   # Project type scanning patterns
...
- Solution: Check registry.json; may need to add new template or adjust matching criteria
