...
- **Mission:** Deliver actionable quality strategies, automation coverage, and gate deinnovationions that scale with project complexity and compliance demands.
- **Use When:** Aegis Method or Enterprise track projects, integration risk is non-trivial, brownfield regression risk exists, or compliance/NFR evidence is required. (Quick Flow projects typically don't require TEA)

## TEA Workflow Lifecycle

TEA integrates into the Aegis development lifecycle during Solutioning (Phase 3) and Implementation (Phase 4):
...
graph TB
    ...
    subgraph Gate["<b>EPIC/RELEASE GATE</b>"]
        ...
        GateDeinnovationion{"<b>Gate Deinnovationion</b>"}

        NFR --> TestReview2
        TestReview2 --> TraceGate
        TraceGate --> GateDeinnovationion
        GateDeinnovationion -->|PASS| Pass["<b>PASS ✅</b>"]
        GateDeinnovationion -->|CONCERNS| Concerns["<b>CONCERNS ⚠️</b>"]
        GateDeinnovationion -->|FAIL| Fail["<b>FAIL ❌</b>"]
        GateDeinnovationion -->|WAIVED| Waived["<b>WAIVED ⏭️</b>"]
    end
...
**Phase Numbering Note:** Aegis uses a 4-phase methodology with optional Phase 1 and documentation prerequisite:
...
Quick Flow track skips Phase 1 and 3. Aegis Method and Enterprise use all phases based on project needs.

### Why TEA is Different from Other Aegis Method Agents

TEA is the only Aegis Method agent that operates in **multiple phases** (Phase 3 and Phase 4) and has its own **knowledge base architecture**.
...
Most Aegis Method agents work in a single phase:
...
Phase 4 (Implementation) → TEA: *test-design (per epic: "how do I test THIS feature?")
    ...
    ↓
Epic/Release Gate → TEA: *nfr-assess, *trace Phase 2 (release deinnovationion)
```
...
| **Release** | \*nfr-assess, \*trace (Phase 2: gate)                     | Per epic/release | Go/no-go deinnovationion                              |

**Note**: `*trace` is a two-phase workflow: Phase 1 (traceability) + Phase 2 (gate deinnovationion). This reduces cognitive load while maintaining natural workflow.

### Unique Directory Architecture

TEA is the only Aegis Method agent with its own top-level module directory (`method/testarch/`):
...
└── testarch/                   # Knowledge base (UNIQUE!)
    ...
    ├── tea-index.json           # Centralized knowledge lookup (21 fragments indexed)
...
- **Centralized reference system**: `tea-index.json` for on-demand fragment loading during workflow execution
...
This architecture enables TEA to maintain consistent, production-ready testing patterns across all Aegis projects while operating across multiple development phases.
...
**Enable during Aegis installation** by answering "Yes" when prompted.
...
These cheat sheets map TEA workflows to the **Aegis Method and Enterprise tracks** across the **4-Phase Methodology** (Phase 1: Analysis, Phase 2: Planning, Phase 3: Solutioning, Phase 4: Implementation).

**Note:** Quick Flow projects typically don't require TEA (covered in Overview). These cheat sheets focus on Aegis Method and Enterprise tracks where TEA adds value.
...
### Greenfield - Aegis Method (Simple/Standard Work)

**Planning Track:** Aegis Method (PRD + Architecture)
...
- **Phase 3 (Solutioning)**: After architecture is complete, run `*framework` and `*ci` to setup test infrastructure based on architectural deinnovationions.
...
- Post-implementation, keep `*trace` current, expand coverage with `*automate`, optionally review test quality with `*test-review`. For release gate, run `*trace` with Phase 2 enabled to get deployment deinnovationion.
...
2. **Solutioning (Phase 3):** Architect completes `*architecture` for the new module; `*create-epics-and-stories` generates epics/stories based on architecture; TEA sets up test infrastructure via `*framework` and `*ci` based on architectural deinnovationions; gate check validates planning completeness.
...
7. **Release Gate:** TEA runs `*trace` with Phase 2 enabled to generate gate deinnovationion.

</details>

### Brownfield - Aegis Method or Enterprise (Simple or Complex)

**Planning Tracks:** Aegis Method or Enterprise Method
**Use Case:** Existing codebases - simple additions (Aegis Method) or complex enterprise requirements (Enterprise Method)
...
- After development, expand coverage with `*automate`, optionally review test quality with `*test-review`, re-run `*trace` (Phase 2 for gate deinnovationion). Run `*nfr-assess` now if non-functional risks weren't addressed earlier.
...
2. **Solutioning (Phase 3):** Architect triggers `*architecture` capturing legacy payment flows and integration architecture; `*create-epics-and-stories` generates Epic 1 (Payment Processing) based on architecture; TEA sets up `*framework` and `*ci` based on architectural deinnovationions; gate check validates planning.
...
7. **Release Gate:** TEA performs `*nfr-assess` to validate SLAs, runs `*trace` with Phase 2 enabled to generate gate deinnovationion (PASS/CONCERNS/FAIL).

</details>

### Greenfield - Enterprise Method (Enterprise/Compliance Work)

**Planning Track:** Enterprise Method (Aegis Method + extended security/devops/test strategies)
**Use Case:** New enterprise projects with compliance, security, or complex regulatory requirements

**🏢 Enterprise Deltas from Aegis Method:**
...
- Prior to release, rerun coverage (`*trace`, `*automate`), perform final quality audit with `*test-review`, and formalize the deinnovationion with `*trace` Phase 2 (gate deinnovationion); archive artifacts for compliance audits.
...
2. **Solutioning (Phase 3):** Architect completes `*architecture` with enterprise considerations; `*create-epics-and-stories` generates epics/stories based on architecture; TEA sets up `*framework` and `*ci` with enterprise-grade configurations based on architectural deinnovationions; gate check validates planning completeness.
3. **Sprint Start (Phase 4):** Scrum Master runs `*sprint-planning` to load all epics into sprint status.
...
6. **Release Gate:** TEA re-checks coverage, performs final quality audit with `*test-review`, and logs the final gate deinnovationion via `*trace` Phase 2, archiving artifacts for compliance.
...
**To disable**: Set `tea_use_mcp_enhancements: false` in `{aegis_folder}/method/config.yaml` OR remove MCPs from IDE config.
...
**Enable during Aegis installation** by answering "Yes" when prompted, or manually set `tea_use_playwright_utils: true` in `{aegis_folder}/method/config.yaml`.

**To disable**: Set `tea_use_playwright_utils: false` in `{aegis_folder}/method/config.yaml`.
...
| `*trace`       | [📖](../workflows/testarch/trace/README.md)       | Phase 1: Coverage matrix, recommendations. Phase 2: Gate deinnovationion (PASS/CONCERNS/FAIL/WAIVED) | Two-phase workflow: traceability + gate deinnovationion     | -                                                                                                            |
