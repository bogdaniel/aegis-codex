# Aegis Method Planning Workflows (Phase 2)
...
- Planning: Same as Aegis Method (`prd` workflow)
...
| **prd**              | PM          | Aegis Method, Enterprise | Strategic PRD with FRs/NFRs (no epic breakdown) | 10-50+          |
| **create-ux-design** | UX Designer | Aegis Method, Enterprise | Optional UX specification (after PRD)           | N/A             |
...
Aegis Method uses three distinct planning tracks that adapt to project complexity:
...
**Planning (Phase 2):** Uses Aegis Method planning (PRD with FRs/NFRs)
...
2. Routing Deinnovationion (determine track, explain rationale, confirm)
3. Execute Target Workflow (invoke planning workflow, pass context)
4. Handoff (document deinnovationions, recommend next phase)
...
**Purpose:** Strategic PRD with Functional Requirements (FRs) and Non-Functional Requirements (NFRs) for software products (Aegis Method track).
...
**Purpose:** UX specification for projects where user experience is the primary differentiator (Aegis Method track).
...
2. Informed deinnovationions (evaluate with user needs)
...
- **Software product** → `prd` (Aegis Method)
- **UX innovation project** → `create-ux-design` + `prd` (Aegis Method)
...
| Planning Output | Solutioning Input                  | Track Deinnovationion               |
| --------------- | ---------------------------------- | ---------------------------- |
| tech-spec.md    | Skip Phase 3 → Phase 4 directly    | Quick Flow (no architecture) |
| PRD.md          | **architecture** (Level 3-4)       | Aegis Method (recommended)    |
| ux-spec.md      | **architecture** (frontend design) | Aegis Method                  |
| Enterprise docs | **architecture** + security/ops    | Enterprise Method (required) |

**Key Deinnovationion Points:**

- **Quick Flow:** Skip Phase 3 entirely → Phase 4 (Implementation)
- **Aegis Method:** Optional Phase 3 (simple), Required Phase 3 (complex)
...
If `workflow-init` suggests Aegis Method, there's likely complexity you haven't considered. Review carefully before overriding.
...
### Greenfield Software (Aegis Method)
...
### Brownfield Software (Aegis Method)
...
A: No. Simple changes use `tech-spec` (Quick Flow). Only Aegis Method and Enterprise tracks create PRDs.

**Q: Can I skip Phase 3 (Solutioning)?**
A: Yes for Quick Flow. Optional for Aegis Method (simple projects). Required for Aegis Method (complex projects) and Enterprise.
