# Aegis Method Brownfield Development Guide
...
| **Aegis Method**       | Feature sets, integrations | 10-50+          | Integrate with existing architecture            |
...
- Keywords: "fix", "bug" → Quick Flow, "dashboard", "platform" → Aegis Method, "enterprise", "multi-tenant" → Enterprise Method
...
```
System: "Found PRD.md (Aegis Method track, 30 stories, 6 months old)"
...
### Documentation Deinnovationion Tree
...
1. **First:** Run `shard-doc` tool to split large files:
   ...
   ```bash
   # Load Aegis Master or any agent
   {aegis_folder}/core/tools/shard-doc.xml --input docs/massive-doc.md
   ```
...
2. **Then:** Run `index-docs` task to create navigation:
   ...
   ```bash
   {aegis_folder}/core/tasks/index-docs.xml --directory ./docs
   ```
...
{aegis_folder}/core/tools/shard-doc.xml \
...
- **PRD** (Aegis Method) can't reference existing code → Designs incompatible features
...
- `product-brief` - Strategic planning (Aegis Method/Enterprise tracks only)

**When to use:** Complex features, technical deinnovationions, strategic additions

**When to skip:** Bug fixes, well-understood features, time-sensitive changes

See the [Workflows section in Aegis Method README](../README.md) for details.
...
**Aegis Method/Enterprise:** Use `prd` workflow
...
### Phase 3: Solutioning (Aegis Method/Enterprise Only)
...
- `create-architecture` - Extend architecture docs (Aegis Method/Enterprise)
- `create-epics-and-stories` - Create epics and stories AFTER architecture
- `implementation-readiness` - Validate before implementation (Aegis Method/Enterprise)
...
### Scenario 3: Feature Set (Aegis Method)
...
### Scenario 4: Complex Integration (Aegis Method)
...
product-brief           # Strategic planning (Aegis Method/Enterprise only)

# Phase 2: Planning (Required)
# PM agent:
tech-spec               # Quick Flow track
prd                     # Aegis Method/Enterprise tracks

# Phase 3: Solutioning (Aegis Method/Enterprise)
# Architect agent:
...
- `docs/method-workflow-status.yaml` - Progress tracker

**Phase 2 Planning:**

- `docs/tech-spec.md` (Quick Flow track)
- `docs/PRD.md` (Aegis Method/Enterprise tracks - FRs/NFRs only)
...
- `docs/architecture.md` (Aegis Method/Enterprise tracks)
...
flowchart TD
    ...
    TRACK -->|Aegis Method| PRD[prd → architecture]
...
5. ✅ **Start with simpler track if unsure** - Easy to upgrade (Quick Flow → Aegis Method)
...
- [GitHub Issues](https://github.com/aegis-code-org/Aegis-METHOD/issues)
- [YouTube Channel](https://www.youtube.com/@AegisCode)

**Documentation:**

- **[Test Architect Guide](./test-architecture.md)** - Comprehensive testing strategy
- [Aegis Method Module README](../README.md) - Complete module and workflow reference
