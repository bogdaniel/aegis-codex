# Aegis Method Troubleshooting Guide

Common issues and solutions for the Aegis Method Module.
...
### Problem: Aegis Method not found after installation

**Symptoms:**

- `aegis` command not recognized
...
```bash
# Check if Aegis Method is installed
ls aegis/
...
# For fresh install
npx aegis@alpha install --skip-version-prompt
```
...
1. Ensure you're loading the correct agent file path: `aegis/method/agents/[agent-name].md`
...
1. Check workflow exists: `ls aegis/method/workflows/`
...
4. **Check status file** - Verify `method-workflow-status.md` or `sprint-status.yaml` is present and valid
...
- Located at `aegis/core/tasks/index-docs.xml`
...
**Solution:**
Check `aegis/method/config.yaml` for configured paths:
...
- Status files: `{output_folder}/method-workflow-status.md`, `{output_folder}/sprint-status.yaml`
...
1. **Check default location**: `docs/method-workflow-status.md`
...
4. **Search for it**:
   ```bash
   find . -name "method-workflow-status.md"
...
**Issue not listed?** Please [report it](https://github.com/aegis-code-org/Aegis-METHOD/issues) so we can add it to this guide!
