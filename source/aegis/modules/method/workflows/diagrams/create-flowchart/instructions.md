...
<workflow>
  ...
  <step n="0" goal="Contextual Analysis (Smart Elicitation)">
    ...
    <action>Extract any mentioned: flowchart type, complexity, deinnovationion points, save location</action>
  ...
  <step n="1" goal="Gather Requirements" elicit="true">
    ...
    <action>Present numbered options:
      ...
      2. Algorithm/Logic Flow - Visualize code logic, deinnovationion trees, or computational processes
    ...
    <action>Present numbered options:
      1. Simple (3-5 steps) - Quick process with few deinnovationion points
      2. Medium (6-10 steps) - Standard workflow with some branching
      3. Complex (11-20 steps) - Detailed process with multiple deinnovationion points
    ...
    <action>Ask Question 3: "Does your flow include deinnovationion points (yes/no branches)?"</action>
    <action>Present numbered options:
      1. No deinnovationions - Linear flow from start to end
      2. Few deinnovationions (1-2) - Simple branching with yes/no paths
      3. Multiple deinnovationions (3-5) - Several conditional branches
      4. Complex deinnovationions (6+) - Extensive branching logic
    </action>
    <action>WAIT for user selection (1-4)</action>
    <action>Store selection in {{deinnovationion_points}}</action>
  ...
  <step n="3" goal="Create Theme" elicit="true">
    ...
    <check if="selection is 5 (Custom)">
      ...
      <action>Ask: "Deinnovationion color (hex code)?"</action>
      <action>WAIT for user input</action>
      <action>Store in {{custom_colors.deinnovationion}}</action>
    </check>
  ...
  <step n="4" goal="Plan Flowchart Layout">
    <action>List all steps and deinnovationion points based on gathered requirements</action>
  ...
  <step n="6" goal="Build Flowchart Elements">
    ...
    <substep>Build Order:
      ...
      3. Each deinnovationion point (diamond) with label
  ...
  <step n="9" goal="Validate Content">
    <invoke-task>Validate against checklist at {{validation}} using {{aegis_folder}}/core/tasks/validate-workflow.xml</invoke-task>
  </step>
