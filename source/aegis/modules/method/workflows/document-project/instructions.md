...
<critical>You MUST have already loaded and processed: {project-root}/{aegis_folder}/method/workflows/document-project/workflow.yaml</critical>
...
<critical>SMART LOADING STRATEGY: Check state file FIRST before loading any JSON files</critical>
...
Your choice [1/2/3]:
    ...
    <check if="user selects 1">
      ...
      <critical>CONDITIONAL JSON LOADING FOR RESUME:</critical>
      <action>For each cached project_type_id, load ONLY the corresponding row from: {documentation_requirements_path}</action>
      <action>Skip loading project-types.json and architecture_registry.json (not needed on resume)</action>
