...
**Workflow ID**: `{aegis_folder}/method/testarch/test-design`
...
1. **Check for sprint-status.yaml**
   - If `{output_folder}/method-sprint-status.yaml` exists → **Epic-Level Mode** (Phase 4)
   - If NOT exists → Check workflow status

2. **Check workflow-status.yaml**
   - Read `{output_folder}/method-workflow-status.yaml`
...
1. **Read Architecture Documentation**
   ...
   - Identify technology stack deinnovationions (frameworks, databases, deployment targets)
...
4. **Load Knowledge Base Fragments (Epic-Level)**
   ...
   **Critical:** Consult `{project-root}/{aegis_folder}/method/testarch/tea-index.json` to load:
   - `risk-governance.md` - Risk classification framework (6 categories: TECH, SEC, PERF, DATA, BUS, OPS), automated scoring, gate deinnovationion engine, owner tracking (625 lines, 4 examples)
   - `probability-impact.md` - Risk scoring methodology (probability × impact matrix, automated classification, dynamic re-assessment, gate integration, 604 lines, 4 examples)
   - `test-levels-framework.md` - Test level selection guidance (E2E vs API vs Component vs Unit with deinnovationion matrix, characteristics, when to use each, 467 lines, 4 examples)
...
2. **Identify Architecturally Significant Requirements (ASRs)**
   ...
   From PRD NFRs and architecture deinnovationions, identify quality requirements that:
   - Drive architecture deinnovationions (e.g., "Must handle 10K concurrent users" → caching architecture)
...
5. **Flag Testability Concerns**
   ...
   Identify architecture deinnovationions that harm testability:
...
- `risk-governance.md` - Risk classification (6 categories), automated scoring, gate deinnovationion engine, coverage traceability, owner tracking (625 lines, 4 examples)
