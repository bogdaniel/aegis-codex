...
<critical>This workflow handles BOTH new projects AND legacy projects following the Aegis Method</critical>

<workflow>

<step n="1" goal="Scan for existing work">
<output>Welcome to Aegis Method, {user_name}!</output>

<action>Perform comprehensive scan for existing work:

- Aegis Method artifacts: PRD, epics, architecture, UX, brief, research, brainstorm
...
- LEGACY: Has code but no Aegis Method artifacts
...
To check progress: Load any Aegis Method agent and run /aegis:method:workflows:workflow-status
...
1. **Aegis Method** - Full planning for complex projects
...
For game development, install the Aegis Gamedev module:
...
<output>Based on your project, here are your Aegis Method planning options:

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**1. Aegis Method** 🎯 {{#if recommended}}(RECOMMENDED){{/if}}
...
<ask>Which Aegis Method approach fits best?
...
<ask if="n">Which Aegis Method track instead?
...
<output>Your Aegis workflow path:
...
<check if="y">
  ...
  <action>Save to {output_folder}/method-workflow-status.yaml</action>
  <action>Identify next workflow and agent</action>

<output>✅ **Created:** {output_folder}/method-workflow-status.yaml

**Next:** {{next_workflow_name}}
**Agent:** {{next_agent}}
**Command:** /aegis:method:workflows:{{next_workflow_id}}

{{#if next_agent not in [analyst, pm]}}
💡 Start new chat with **{{next_agent}}** agent first.
{{/if}}

To check progress: /aegis:method:workflows:workflow-status
