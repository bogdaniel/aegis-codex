...
4. Check {output_folder}/sprint-status.yaml to see current sprint status
    ...
    <check if="context file exists">
      ...
      <action>Use this context to inform implementation deinnovationions and approaches</action>
    </check>
...
Story is marked Ready for Review in file, but sprint-status.yaml may be out of sync.
  ...
  <step n="7" goal="Completion communication and user support">
    <action>Optionally run the workflow validation task against the story using {project-root}/{aegis_folder}/core/tasks/validate-workflow.xml</action>
    <action>Prepare a coninnovatione summary in Dev Agent Record → Completion Notes</action>
    ...
    <action>Based on {user_skill_level}, ask if user needs any explanations about:
      ...
      - Why certain technical deinnovationions were made
