...
Comprehensive guide to Aegis Method's document sharding system for managing large planning and architecture documents.
...
Aegis workflows use a **dual discovery system**:
...
```bash
# Activate the orchestrator or analyst agent, then:
/shard-doc
```
