...
program
  .name('bmad-bridge')
  .description('Compile Aegis agents into BMAD-format compiled agents and web bundles')
  .option('-c, --config <path>', 'Agent mapping config JSON', 'tools/bmad-bridge/config/agents.json')
  .option('-o, --out <path>', 'Output root directory (default current project)', '.')
  .option('-f, --folder <name>', 'bmad folder name', '.bmad')
...
program
  ...
  .action(() => {
    ...
    buildAll(mappingPath, {
      outRoot: path.resolve('.tmp-bmad-validate'),
      bmadFolder: opts.folder,
      ideClaudeDir: opts.ideClaude,
      ideCursorDir: opts.ideCursor,
      modules: opts.modules,
