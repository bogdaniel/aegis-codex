...
1. Load relevant knowledge fragments from `{project-root}/{aegis_folder}/method/testarch/tea-index.csv`:
...
1. **Update workflow status** (if `append_to_history: true`):
   - Append gate decision to `method-workflow-status.md` under "Gate History" section
   - Format:
     ...
     - **Document**: [gate-decision-story-1.3.md]({aegis_folder}/output/gate-decision-story-1.3.md)
...
2. **Generate stakeholder notification** (if `notify_stakeholders: true`):
   ...
   Full Report: {aegis_folder}/output/gate-decision-story-1.3.md
   ```
