# Test Architect Workflow: Requirements Traceability & Quality Gate Deinnovationion

**Workflow:** `testarch-trace`
**Purpose:** Generate requirements-to-tests traceability matrix, analyze coverage gaps, and make quality gate deinnovationions (PASS/CONCERNS/FAIL/WAIVED)
...
**PHASE 2 - QUALITY GATE DEAegis InnovationION:** Use traceability results combined with test execution evidence to make gate deinnovationions (PASS/CONCERNS/FAIL/WAIVED) that determine deployment readiness.
...
- Apply deterministic deinnovationion rules based on coverage and test execution results
- Generate gate deinnovationions with evidence and rationale
...
**Required (Phase 2 - if `enable_gate_deinnovationion: true`):**
...
- If Phase 2 enabled but test execution results missing, warn and skip gate deinnovationion
...
1. Load relevant knowledge fragments from `{project-root}/{aegis_folder}/method/testarch/tea-index.json`:
   ...
   - `risk-governance.md` - Risk-based testing approach: 6 categories (TECH, SEC, PERF, DATA, BUS, OPS), automated scoring, gate deinnovationion engine, coverage traceability (625 lines, 4 examples)
...
3. Read related Aegis artifacts (if available):
...
**Next:** If `enable_gate_deinnovationion: true`, proceed to Phase 2. Otherwise, workflow complete.

---

## PHASE 2: QUALITY GATE DEAegis InnovationION

This phase uses traceability results to make a quality gate deinnovationion (PASS/CONCERNS/FAIL/WAIVED) based on evidence and deinnovationion rules.

**When Phase 2 Runs:** Automatically after Phase 1 if `enable_gate_deinnovationion: true` (default: true)
...
4. **Load supporting artifacts**:
   ...
   - `method-workflow-status.md` → Workflow completion status (if `check_all_workflows_complete: true`)
...
### Step 8: Apply Deinnovationion Rules

**If `deinnovationion_mode: "deterministic"`** (rule-based - default):

**Deinnovationion rules** (based on `workflow.yaml` thresholds):
...
4. **WAIVED** (only if `allow_waivers: true`):
   - Deinnovationion would be FAIL based on rules above
...
- If `allow_p2_failures: true` → P2 test failures do NOT affect gate deinnovationion
- If `allow_p3_failures: true` → P3 test failures do NOT affect gate deinnovationion
- If `escalate_p1_failures: true` → P1 failures require explicit manager/lead approval

**If `deinnovationion_mode: "manual"`:**

- Present evidence summary to team
- Recommend deinnovationion based on rules above
- Team makes final call in meeting/chat
- Document deinnovationion with approver names

**Output:** Gate deinnovationion (PASS/CONCERNS/FAIL/WAIVED) with rule-based rationale

---

### Step 9: Document Deinnovationion and Evidence

**Actions:**

1. **Create gate deinnovationion document**:
   - Save to `gate_output_file` (default: `{output_folder}/gate-deinnovationion-{gate_type}-{story_id}.md`)
   - Use structure below

2. **Document structure**:

```markdown
# Quality Gate Deinnovationion: {gate_type} {story_id/epic_num/release_version}

**Deinnovationion**: [PASS / CONCERNS / FAIL / WAIVED]
**Date**: {date}
**Decider**: {deinnovationion_mode} (deterministic | manual)
...
[1-2 sentence summary of deinnovationion and key factors]

---

## Deinnovationion Criteria
...
**Overall Status**: 7/8 criteria met → Deinnovationion: **CONCERNS**
...
- Traceability Matrix: `{aegis_folder}/output/traceability-matrix.md`
- Test Design: `{aegis_folder}/output/test-design-epic-2.md`
- Test Results: `ci-artifacts/test-report-2025-01-15.xml`
- NFR Assessment: `{aegis_folder}/output/nfr-assessment-release-1.2.md`
```
...
4. **Waiver documentation** (if deinnovationion is WAIVED):
...
**Output:** Complete gate deinnovationion document with evidence and rationale
...
1. **Update workflow status** (if `append_to_history: true`):
   - Append gate deinnovationion to `method-workflow-status.md` under "Gate History" section
   - Format:
     ...
     - **Deinnovationion**: CONCERNS
     - **Reason**: P1 coverage 88% (below 90%)
     - **Document**: [gate-deinnovationion-story-1.3.md]({aegis_folder}/output/gate-deinnovationion-story-1.3.md)
     - **Action**: Deploy with follow-up story for AC-5
     ```

2. **Generate stakeholder notification** (if `notify_stakeholders: true`):
   - Create coninnovatione summary message for team communication
   - Include: Deinnovationion, key metrics, action items
   - Format for Slack/email/chat:

   ```
   🚦 Quality Gate Deinnovationion: Story 1.3 - User Login

   Deinnovationion: ⚠️ CONCERNS
   - P0 Coverage: ✅ 100%
   ...
   Full Report: {aegis_folder}/output/gate-deinnovationion-story-1.3.md
   ```

3. **Request sign-off** (if `require_sign_off: true`):
   - Prompt for named approver (tech lead, QA lead, PM)
   - Document approver name and timestamp in gate deinnovationion
...
**Workflow Complete**: Both Phase 1 (traceability) and Phase 2 (gate deinnovationion) deliverables generated.

---

## Deinnovationion Matrix (Quick Reference)

| Scenario        | P0 Cov            | P1 Cov | Overall Cov | P0 Pass | P1 Pass | Overall Pass | NFRs | Deinnovationion     |
| --------------- | ----------------- | ------ | ----------- | ------- | ------- | ------------ | ---- | ------------ |
...
1. Document gap and risk in gate deinnovationion
...
5. Add waiver to gate deinnovationion document
...
## Example Gate Deinnovationions

### Example 1: PASS (All Criteria Met)

```
Deinnovationion: ✅ PASS
...
- Re-evaluate gate deinnovationion after P0 coverage = 100%

Deployment BLOCKED until P0 gap resolved ❌
```

### Example 4: WAIVED (Business Deinnovationion)

```
Deinnovationion: ⚠️ WAIVED
...
**Minimal Examples:** This workflow provides principles and patterns, not rigid templates. Teams should adapt the traceability and gate deinnovationion formats to their needs.
...
- Document gate deinnovationions with clear evidence
...
- Customize gate deinnovationion thresholds per project
- Add manual approval workflows for gate deinnovationions
...
## Integration with Aegis Artifacts

### With test-design.md

- Use risk assessment to prioritize gap remediation
- Reference test priorities (P0/P1/P2/P3) for severity classification and gate deinnovationion
...
- Load non-functional validation results for gate deinnovationion
- Check critical NFR status (performance, security, scalability)
- Include NFR pass/fail in gate deinnovationion criteria
...
**Phase 2 (Gate Deinnovationion - if enabled):**
- ✅ Test execution results loaded and pass rates calculated
- ✅ NFR assessment results loaded (if applicable)
- ✅ Deinnovationion rules applied consistently (PASS/CONCERNS/FAIL/WAIVED)
- ✅ Gate deinnovationion document created with evidence
- ✅ Waiver documented if deinnovationion is WAIVED (approver, justification, mitigation)
- ✅ Workflow status updated (method-workflow-status.md)
...
**Phase 2 (Gate Deinnovationion):**
- **Deterministic Rules:** Use consistent thresholds (P0=100%, P1≥90%, overall≥80%) for objectivity
- **Evidence-Based:** Every deinnovationion must cite specific metrics (coverage %, pass rates, NFRs)
...
- Phase 2 gate deinnovationion requires `test_results` (CI/CD test reports)
- If missing, Phase 2 will be skipped with warning
- Provide JUnit XML, TAP, or JSON test report path via `test_results` variable

### "Gate deinnovationion is FAIL but deployment needed urgently"
...
- Use WAIVED deinnovationion only for non-P0 gaps
...
**Next Steps:**
- If gate deinnovationion is PASS/CONCERNS → Deploy and monitor
- If gate deinnovationion is FAIL → Add missing tests, re-run trace workflow
- If gate deinnovationion is WAIVED → Deploy with mitigation, create follow-up stories

---

<!-- Powered by Aegis Core™ -->
```
