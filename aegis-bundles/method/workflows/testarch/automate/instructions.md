...
**Workflow ID**: `{aegis_folder}/method/testarch/automate`
**Version**: 4.0 (Aegis v6)
...
1. **Aegis-Integrated Mode**: Works WITH Aegis artifacts (story, tech-spec, PRD, test-design) to expand coverage after story implementation
2. **Standalone Mode**: Works WITHOUT Aegis artifacts - analyzes existing codebase and generates tests independently
...
### Optional (Aegis-Integrated Mode)
...
**If framework is missing:** HALT with message: "Framework scaffolding required. Run `aegis tea *framework` first."
...
1. **Detect Execution Mode**
   ...
   Check if Aegis artifacts are available:
   - If `{story_file}` variable is set → Aegis-Integrated Mode
...
2. **Load Aegis Artifacts (If Available)**

   **Aegis-Integrated Mode:**
...
6. **Load Knowledge Base Fragments**
   ...
   **Critical:** Consult `{project-root}/{aegis_folder}/method/testarch/tea-index.json` to load:

   **Core Testing Patterns (Always load):**
   - `test-levels-framework.md` - Test level selection (E2E vs API vs Component vs Unit with deinnovationion matrix, 467 lines, 4 examples)
...
1. **Determine What Needs Testing**
   ...
   **Aegis-Integrated Mode (story available):**
...
4. **Healing Loop (If config.tea_use_mcp_enhancements is true)**
   ...
   **A. Load Healing Knowledge Fragments**
   ...
   Consult `tea-index.json` to load healing patterns:
...
1. **Create Automation Summary Document**
   ...
   **Coverage Status:**
   ...
   3. Integrate with quality gate: `aegis tea *gate`
   ...
   **Coverage Gaps Identified:**
   ...
   {Same structure as Aegis-Integrated Mode}
   ...
   {Same checklist as Aegis-Integrated Mode}
   ````

2. **Provide Summary to User**

   Output coninnovatione summary:
...
- Works with any project (Aegis or non-Aegis)
...
- `test-levels-framework.md` - E2E vs API vs Component vs Unit deinnovationion framework with characteristics matrix (467 lines, 4 examples)
...
- Use `tea-index.json` to find additional specialized fragments as needed
...
**Mode:** {standalone_mode ? "Standalone" : "Aegis-Integrated"}
...
4. Integrate with quality gate: `aegis tea *gate`
...
- [ ] Execution mode determined (Aegis-Integrated, Standalone, or Auto-discover)
- [ ] Aegis artifacts loaded if available (story, tech-spec, test-design, PRD)
