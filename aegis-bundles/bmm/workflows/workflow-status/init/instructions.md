...
To check progress: Load any BMM agent and run /aegis:bmm:workflows:workflow-status
...
**Agent:** {{next_agent}}
**Command:** /aegis:bmm:workflows:{{next_workflow_id}}

{{#if next_agent not in [analyst, pm]}}
💡 Start new chat with **{{next_agent}}** agent first.
{{/if}}

To check progress: /aegis:bmm:workflows:workflow-status
