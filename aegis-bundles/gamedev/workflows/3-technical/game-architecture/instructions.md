# Deinnovationion Architecture Workflow Instructions

<workflow name="architecture">

<critical>The workflow execution engine is governed by: {project-root}/{aegis_folder}/core/tasks/workflow.xml</critical>
<critical>You MUST have already loaded and processed: {installed_path}/workflow.yaml</critical>
<critical>This workflow uses ADAPTIVE FACILITATION - adjust your communication style based on {user_skill_level}</critical>
<critical>The goal is ARCHITECTURAL DEAegis InnovationIONS that prevent AI agent conflicts, not detailed implementation specs</critical>
...
<critical>ELICITATION POINTS: After completing each major architectural deinnovationion area (identified by template-output tags for deinnovationion_record, project_structure, novel_pattern_designs, implementation_patterns, and architecture_document), invoke advanced elicitation to refine deinnovationions before proceeding</critical>
...
<action>Check if {output_folder}/method-workflow-status.yaml exists</action>

<check if="status file not found">
  <output>No workflow status file found. Deinnovationion Architecture can run standalone or as part of Aegis Method workflow path.</output>
...
<check if="status file found">
  <action>Load the FULL file: {output_folder}/method-workflow-status.yaml</action>
...
Deinnovationion Architecture works from your Product Requirements Document (PRD).
...
{{if_ux_spec}}I also found your UX specification which defines the user experience requirements.{{/if_ux_spec}}
     ...
     This will help me guide you through the architectural deinnovationions needed
...
<step n="2" goal="Discover and evaluate starter templates">
  <critical>Modern starter templates make many good architectural deinnovationions by default</critical>
...
Search the web: "{{primary_technology}} boilerplate generator latest options"
  ...
  <check if="starter_templates_found">
    ...
    <check if="{user_skill_level} == 'expert'">
      <action>Present starter options coninnovationely:
        "Found {{starter_name}} which provides:
         {{quick_deinnovationion_list}}
    ...
    <check if="{user_skill_level} == 'beginner'">
      <action>Explain starter benefits:
        "I found {{starter_name}}, which is like a pre-built foundation for your project.
         ...
         It makes these deinnovationions for you:
         {{friendly_deinnovationion_list}}
    ...
    <check if="user_accepts_starter">
      ...
      <action>Extract and document starter-provided deinnovationions:
        Starter provides these architectural deinnovationions:
      ...
      <action>Mark these deinnovationions as "PROVIDED BY STARTER" in our deinnovationion tracking</action>
    ...
    <check if="user_rejects_starter">
      ...
      <action>Note: Manual setup required, all deinnovationions need to be made explicitly</action>
    </check>
  ...
  <check if="no_starter_found_or_applicable">
    <action>Note: No standard starter template found for this project type.
            We will make all architectural deinnovationions explicitly.</action>
  </check>

<template-output>starter_template_deinnovationion</template-output>
</step>

<step n="3" goal="Adapt facilitation style and identify remaining deinnovationions">
  <action>Based on {user_skill_level} from config, set facilitation approach:

  <check if="{user_skill_level} == 'expert'">
    Set mode: EXPERT
    - Use technical terminology freely
    - Move quickly through deinnovationions
  ...
  <check if="{user_skill_level} == 'beginner'">
    ...
    - Provide education about why deinnovationions matter
    - Protect from complexity overload
  </check>
  </action>

<action>Load deinnovationion catalog: {deinnovationion_catalog}</action>
<action>Load architecture patterns: {architecture_patterns}</action>

<action>Analyze PRD against patterns to identify needed deinnovationions: - Match functional requirements to known patterns - Identify which categories of deinnovationions are needed - Flag any novel/unique aspects requiring special attention - Consider which deinnovationions the starter template already made (if applicable)
</action>

<action>Create deinnovationion priority list:
CRITICAL (blocks everything): - {{list_of_critical_deinnovationions}}

    IMPORTANT (shapes architecture):
    - {{list_of_important_deinnovationions}}

    NICE-TO-HAVE (can defer):
    - {{list_of_optional_deinnovationions}}

  </action>

<action>Announce plan to {user_name} based on mode:
<check if="mode == 'EXPERT'">
"Based on your PRD, we need to make {{total_deinnovationion_count}} architectural deinnovationions.
{{starter_covered_count}} are covered by the starter template.
Let's work through the remaining {{remaining_count}} deinnovationions."
</check>

    <check if="mode == 'BEGINNER'">
      "Great! I've analyzed your requirements and found {{total_deinnovationion_count}} technical
...
<template-output>deinnovationion_identification</template-output>
</step>

<step n="4" goal="Facilitate collaborative deinnovationion making" repeat="for-each-deinnovationion">
  <critical>Each deinnovationion must be made WITH the user, not FOR them</critical>
  <critical>ALWAYS search the web to verify current versions - NEVER trust hardcoded versions</critical>

<action>For each deinnovationion in priority order:</action>

<action>Present the deinnovationion based on mode:
<check if="mode == 'EXPERT'">
"{{Deinnovationion_Category}}: {{Specific_Deinnovationion}}
  ...
  <check if="deinnovationion_involves_specific_technology">
    ...
    <action>Update deinnovationion record with verified version:
  ...
  <check if="user_wants_more_info">
    ...
    <check if="complex_tradeoffs">
      <action>Consider using advanced elicitation:
        "Would you like to explore innovative approaches to this deinnovationion?
         I can help brainstorm unconventional solutions if you have specific goals."
      </action>
    </check>
  </check>

<action>Record deinnovationion:
Category: {{category}}
...
"This choice means we'll also need to {{related_deinnovationions}}"
</action>

<template-output>deinnovationion_record</template-output>
</step>

<step n="5" goal="Address cross-cutting concerns">
  <critical>These deinnovationions affect EVERY epic and story</critical>

<action>Facilitate deinnovationions for consistency patterns: - Error handling strategy (How will all agents handle errors?) - Logging approach (Structured? Format? Levels?) - Date/time handling (Timezone? Format? Library?) - Authentication pattern (Where? How? Token format?) - API response format (Structure? Status codes? Errors?) - Testing strategy (Unit? Integration? E2E?)
</action>
...
<template-output>cross_cutting_deinnovationions</template-output>
</step>

<step n="6" goal="Define project structure and boundaries">
  <action>Based on all deinnovationions made, define the project structure</action>
...
<action>For each relevant pattern category, facilitate deinnovationions:
  ...
  <check if="{user_skill_level} == 'expert'">
    <action>Rapid-fire through patterns:
      "Quick deinnovationions on implementation patterns:
...
<action>Check deinnovationion compatibility: - Do all deinnovationions work together? - Are there any conflicting choices? - Do the versions align properly?
</action>

<action>Verify epic coverage: - Does every epic have architectural support? - Are all user stories implementable with these deinnovationions? - Are there any gaps?
</action>
...
<action>Validate pattern completeness: - Are there any patterns we missed that agents would need? - Do novel patterns integrate with standard architecture? - Are implementation patterns comprehensive enough?
  ...
  <check if="issues_found">
    ...
    <action>Update deinnovationions based on resolution</action>
  </check>

<template-output>coherence_validation</template-output>
</step>

<step n="10" goal="Generate deinnovationion architecture document">
...
"## Project Initialization
       ...
       This establishes the base architecture with these deinnovationions:
       {{starter_provided_deinnovationions}}"
    </check>
...
□ Deinnovationion table has Version column with specific versions
...
<step n="12" goal="Final review and update workflow status">
  ...
  <check if="{user_skill_level} == 'expert'">
    "Architecture complete. {{deinnovationion_count}} deinnovationions documented.
     Ready for implementation phase."
  </check>

  <check if="{user_skill_level} == 'beginner'">
    "Excellent! Your architecture is complete. You made {{deinnovationion_count}} important
     deinnovationions that will keep AI agents consistent as they build your app.
...
<action>Save document to {output_folder}/architecture.md</action>
  ...
  <check if="standalone_mode != true">
    ...
    <action>Update workflow_status["create-architecture"] = "{output_folder}/method-architecture-{{date}}.md"</action>
...
<output>✅ Deinnovationion Architecture workflow complete!</output>

<output>**Deliverables Created:**

- ✅ architecture.md - Complete architectural deinnovationions document
  {{if_novel_patterns}}
