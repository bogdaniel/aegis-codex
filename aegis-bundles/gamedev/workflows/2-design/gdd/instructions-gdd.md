...
<critical>The workflow execution engine is governed by: {project_root}/{aegis_folder}/core/tasks/workflow.xml</critical>
...
<critical>Uses gdd_template for GDD output, game_types.json for type-specific sections</critical>
<critical>Routes to 3-solutioning for architecture (platform-specific deinnovationions handled there)</critical>
<critical>If users mention technical details, append to technical_preferences with timestamp</critical>

<critical>DOCUMENT OUTPUT: Coninnovatione, clear, actionable game design specs. Use tables/lists over prose. User skill level ({user_skill_level}) affects conversation style ONLY, not document content.</critical>
...
<invoke-workflow path="{project-root}/{aegis_folder}/method/workflows/workflow-status">
...
The GDD workflow can run standalone or as part of the Aegis Method workflow path.
...
<action>Map to closest game_types.json id or use "custom"</action>
</check>
...
<action>Map selection to game_types.json id</action>
...
<critical>These are game-defining deinnovationions</critical>
...
<check if="standalone_mode != true">
  ...
  <action>Update workflow_status["gdd"] = "{output_folder}/method-gdd-{{game_name}}-{{date}}.md"</action>
...
<check if="user selects option 1 or fuzzy indicates wanting to create the narrative design document">
  <invoke-workflow>{project-root}/{aegis_folder}/method/workflows/2-plan-workflows/narrative/workflow.yaml</invoke-workflow>
...
- Generate architecture.md with engine-specific deinnovationions
- Create per-epic tech specs
- Handle platform-specific architecture (from registry.json game-\* entries)
...
- [ ] **Run solutioning workflow** (REQUIRED)
  ...
  - Input: GDD.md, method-workflow-status.md
  - Output: architecture.md with engine/platform specifics
  - Note: Registry.json will provide engine-specific guidance
